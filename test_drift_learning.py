...
# ux, du_dx = celldelta.phi.u.div(x)
...
high = float(X.max())
l = low - 0.25 * (high - low)
h = high + 0.25 * (high - low)
xs = torch.arange(l, h, 0.01, device=device)[:, None]
phi = tonp(model.phi(xs))

model.eval()
pxts = np.exp(model.pxt.log_pxt(xs, ts).squeeze().T.cpu().detach().numpy())
...
ax1.plot(xs[1000:], uxs[1000:], c="blue", alpha=0.6)
...
ax2 = ax1.twinx()
data_dist, data_bins = np.histogram(
    X.detach().cpu().numpy().flatten(), bins=150, density=True
)
w = data_bins[1] - data_bins[0]
ax2.bar(data_bins[:-1], data_dist, width=w, alpha=0.3, label="Data")
ax2.set_ylabel("p(x)")
fig.legend()
...
axs[0].set_ylabel("x")
axs[0].set_yticks(
    ticks=np.linspace(0, sim_cum_pxt.shape[0], 10),
    labels=np.round(np.linspace(xs.min(), xs.max(), 10), 2),
)
axs[0].set_xlabel("timestep (t)")
axs[0].set_xticks(
    ticks=np.linspace(0, sim_cum_pxt.shape[1], 10),
...
xts = model.simulate(x, ts, zero_boundary=zero_boundary, sigma=0)
...
for i in np.linspace(0, ts.shape[0] - 1, 20, dtype=int):
    t = ts[i]
    heights, bins = np.histogram(xts[i, :], bins=bins, density=True)
    plt.bar(
        bins[:-1], heights, width=w, color=viridis(float(t) / max(ts).item()), alpha=0.2
    )

for i in np.linspace(0, ts.shape[0] - 1, 20, dtype=int):
    plt.plot(xs, pxts[:, i], color="blue", alpha=0.2)

labels = ["Simulation", "Fokker-Planck theoretical"]
artists = [plt.Line2D([0], [0], color=c, alpha=0.2) for c in ["red", "blue"]]
...
plt.bar(sim_mean_bins[:-1], sim_mean_dist, width=w, alpha=0.3, label="Simulation")

plt.bar(data_bins[:-1], data_dist, width=w, alpha=0.3, label="Data")
plt.ylabel("p(x)")
plt.xlabel("x")
plt.legend()
# %%
...
plt.bar(data_bins[:-1], data_dist, width=w, alpha=0.5, label="Data", color="green")
plt.plot(
    tonp(xs), pxts.mean(axis=1), alpha=0.4, label="Estimated p(x,t)", color="purple"
)
plt.legend()
# %%
# Plot the histogram of differences from the true pseudotime
fig, axs = plt.subplots(4, 1, figsize=(10, 20))
pXt = model.pxt.pxt(X, ts).squeeze().cpu().detach().numpy()
...
axs[1].set_xticks(
    np.arange(diffs.min(), diffs.max(), 10) + 0.5,
    labels=np.arange(diffs.min(), diffs.max(), 10),
    rotation=90,
)
...
pct95 = abs_diffs[np.where(np.arange(0, n) / n > 0.95)[0][0]]
axs[2].axvline(pct95, c="red", linestyle="--", linewidth=1)
# Label the 95th percentile, pad the label so it doesn't overlap with the line
axs[2].text(pct95 + abs_diffs.max() / 100, 0, f"95th pct={pct95:d}")
axs[2].set_title(
    "Empirical CDF of absolute differences of estimated versus true pseudotime"
)
...
# Plot all the cells
axs[0].scatter(x_proj[:, 0], x_proj[:, 1], c="grey", s=0.5, alpha=0.5)
...
for i in range(n_cells):
    axs[0].arrow(
        random_cells_proj[i, 0],
        random_cells_proj[i, 1],
...
xts = model.simulate(
    x,
    tsim=torch.linspace(0, max_t * tscale, 100, device=device) * tscale,
...
for i in range(3):
    for j in range(3):
        ...
        axs[i, j].scatter(x_proj[:, 0], x_proj[:, 1], c="grey", s=0.5, alpha=0.5)
        # Plot the sequence of points in the trajectory, colored by their timestep, connected by lines
        axs[i, j].plot(traj_proj[:, 0], traj_proj[:, 1], c="black", alpha=0.5)
        axs[i, j].scatter(
            traj_proj[:, 0], traj_proj[:, 1], c=tsim_np[:], cmap=viridis, s=1.5
        )
        axs[i, j].set_xticks([])
        axs[i, j].set_yticks([])
...
if scatter:
    ...
    plt.scatter(
        xts_proj[:, 0], xts_proj[:, 1], s=0.5, alpha=0.5, c=t_idxs, cmap=viridis
    )
# %%
X.requires_grad = True
model = model.eval()
dq_dx, dq_dt = model.pxt.dx_dt(X, ts)
ux, du_dx = model.phi.u.div(X)
d_dx = ((dq_dx * ux).sum(dim=2) + du_dx)[..., None]
X.requires_grad = False
# %%0
x0n1 = X_t[:, :].reshape((-1, d)).clone().detach()
pxt0 = model.pxt.log_pxt(x0n1, ts).squeeze().T.cpu().detach().numpy()
...
for i in range(5):
    ...
    for j in range(n_cells):
        ...
        arrow = matplotlib.patches.FancyArrowPatch(
            (x, y), (dx, dy), color="red", alpha=0.5, mutation_scale=10
        )
        arrowstyle = matplotlib.patches.ArrowStyle.Simple(
            head_length=0.5, head_width=0.25, tail_width=0.01
        )
...
axs[5][0].scatter(
    x_proj[:, 0], x_proj[:, 1], c=tonp((dq_dx**2).sum((0, 2))), cmap="viridis", s=1
)
plt.colorbar(axs[5][0].collections[0], ax=axs[5][0])
axs[5][1].scatter(
    x_proj[:, 0], x_proj[:, 1], c=tonp((dq_dt**2).sum(0)), cmap="viridis", s=1
)
...
dq_dx, dq_dt = model.pxt.dx_dt(x, ts)
u = model.phi.u.model(x)
div_ux = torch.zeros_like(x[:, 0])
for i in range(u.shape[1]):
    div_ux += torch.autograd.grad(
        u[:, i], x, torch.ones_like(u[:, i]), retain_graph=True, create_graph=True
    )[0][:, i]
u.retain_grad()
dq_dx.retain_grad()
...
for name, param in model.phi.u.model.named_parameters():
